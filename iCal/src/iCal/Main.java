...
public class Main {
	...
	public static void displayMenu() throws IOException {
		...
		do {
			...
			try {
				...
				else if (choice.equalsIgnoreCase("Import")
						|| choice.equals("2")) {
					System.out
							.print("Please enter the name of your file (Do not include .ics)\n: ");
					String fileName = keyboard.nextLine();
					// check if input contains .ics at the end
					// int fileNameLength = fileName.length();
					String fileNameEnding = fileName.substring(fileName
							.length() - 4); // last four char of fileName
	...
	public static Event gatherData() throws IOException {
		...
		// Event's start date
		System.out
				.print("Enter the event's start date in the format YYYYMMDD: ");
		...
		// Event's start time
		System.out
				.print("Enter event's start time in 24 hr format (e.g. 4 pm = 1600): ");
		event1.setTimeStart(checkValidTime()); // calls method below to validate
												// time entered is in correct
												// format

		// Event's end time
		System.out
				.print("Enter event's ending time in 24 hr format (e.g. 4 pm = 1600): ");
		event1.setTimeEnd(checkValidTime()); // calls method below to validate
												// time entered is in correct
												// format
		...
		while (!proceed) {
			if (tzidAnswer.charAt(0) == 'Y' || tzidAnswer.charAt(0) == 'y') {
				displayTimeZone();
				System.out
						.print("Please select a timezone (enter numbers only)\n: ");

				event1.setTzid(scan.nextLine());
				proceed = true;
			} else if (tzidAnswer.charAt(0) == 'n'
					|| tzidAnswer.charAt(0) == 'n') {
				proceed = true;
			} else {
				System.out
						.print("Type \'Y\' or \'N\' to whether you want to set the Timezone:");
		...
		// Location
		System.out
				.print("Enter event's location (optional; press Enter to continue): ");
		event1.setLocation(scan.nextLine());

		// GEO
		System.out
				.print("Would you like to enter a latitude and longitude? (Enter Y/N): ");
			...
			if (geoAnswer.charAt(0) == 'Y' || geoAnswer.charAt(0) == 'y') {
				...
				do {
					...
					while (!scan.hasNextFloat()) {
						scan.next();
						System.out
								.println("\nLatitude must be specified by a number between -90 to 90.");
						System.out
								.println("South is indicated by a minus sign (-).");
						System.out
								.println("North is indicated by a plus sign (+) or the absence of a minus sign (-).");
						System.out.print("Please retry entering a latitude: ");
					}
					latInput = scan.nextFloat();
					if (Math.abs(latInput) > 90) {
						System.out
								.println("\nLatitude must be specified by a number between -90 to 90.");
						System.out
								.println("South is indicated by a minus sign (-).");
				...
				event1.setLatitude(latInput); // truncate to 6 decimal places on
												// output
				do {
					...
					while (!scan.hasNextFloat()) {
						scan.next();
						System.out
								.println("\nLongitude must be specified by a number between -180 to 180.");
						System.out
								.println("West is indicated by a minus sign (-).");
						System.out
								.println("East is indicated by a plus sign (+) or the absence of a minus sign (-).");
						System.out.print("Please retry entering a longitude: ");
					}
					lonInput = scan.nextFloat();
					if (Math.abs(lonInput) > 180) {
						System.out
								.println("\nLongitude must be specified by a number between -180 to 180.");
						System.out
								.println("West is indicated by a minus sign (-).");
				...
				event1.setLongitude(lonInput); // truncate to 6 decimal places
												// on output
				proceed = true;
		...
		String[] classes = { "PUBLIC", "PRIVATE", "CONFIDENTIAL", "iana-token",
				"x-name" };
		System.out.print("\n");
		int classInput = -1;
		while (classInput < 0 || classInput > 4) {
			...
			System.out
					.print("Classify this event by entering a number from the list above: "); //
			classInput = scan.nextInt();
			if (classInput < 0 || classInput > 4)
				System.out
						.println("Invalid input. Enter a number from the following list:");
		}
	...
	public static String checkValidTime() {
		...
		do {
			...
			if (!(isValidTime(tempTime))) {
				...
				System.out
						.print("Please enter the time in 24 hr format (e.g. 4 pm = 1600): ");
			}
	...
	public static String checkValidDate() {
		...
		do {
			...
			if (!(isValidDate(tempDate))) {
				...
				System.out
						.print("Enter the event's date in the format YYYYMMDD: ");
			}
	...
	private static void writeIcs(Event event1) throws IOException {
		...
		pw.printf("%s%n", "CREATED:" + event1.getDateCreated());// time created
																// stamp

		pw.printf("%s%s%n", "DESCRIPTION:", event1.getDescription());// description
		pw.printf("%s%S%n", "COMMENT:", event1.getComment()); // comment
		// pw.printf("%s%s%n", "LAST-MODIFIED:",
		// event1.getDateModified());//time modified stamp -- we don't need
		// last-modified, same as dtstamp
		pw.printf("%s%s%n", "LOCATION:", event1.getLocation());// location
																// variable
		pw.printf("%s%.6f;%.6f%n", "GEO:", event1.getLatitude(),
				event1.getLongitude());// need this for assignment requirement
		...
		System.out.println("Finished saving to " + event1.getFileName()
				+ ".ics!");
		pw.close();
	...
	public static String pickFilename(Event e) {
		System.out
				.print("Enter a name for your .ics file(.ics will be appended): ");
